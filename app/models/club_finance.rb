  ...
  def self.updateEarnings(game, action, description = nil)
    ...
    if action == "confirm"
      ...
      if game.hscore.to_i > game.vscore.to_i
        homeEarnings += game.championship.preferences["match_winning_earning"].delete(",").to_i
        homeDescription.push("Win")
        visitorEarnings -= game.championship.preferences["match_lost_earning"].delete(",").to_i
        visitorDescription.push("Lost")
      elsif game.hscore.to_i < game.vscore.to_i
        homeEarnings -= game.championship.preferences["match_lost_earning"].delete(",").to_i
        homeDescription.push("Lost")
        visitorEarnings += game.championship.preferences["match_winning_earning"].delete(",").to_i
      ...
      if game.wo == false
        ...
        homeGoals.each do |hGoals|
          if hGoals[1] >= 3
            if homeHT == false
              homeHT = true
              homeEarnings += game.championship.preferences["hattrick_earning"].delete(",").to_i
        ...
        visitorGoals.each do |vGoals|
          if vGoals[1] >= 3
            if visitorHT == false
              visitorHT = true
              visitorEarnings += game.championship.preferences["hattrick_earning"].delete(",").to_i
        ...
        # Draw Earning
        if game.hscore.to_i == game.vscore.to_i
          homeEarnings += game.championship.preferences["match_draw_earning"].delete(",").to_i
          homeDescription.push("Draw")

          visitorEarnings += game.championship.preferences["match_draw_earning"].delete(",").to_i
          visitorDescription.push("Draw")
        end

        # Goal Earning
        if game.hscore.to_i > 0 && game.wo == false
          homeEarnings += (game.hscore.to_i * game.championship.preferences["match_goal_earning"].delete(",").to_i)
          homeDescription.push("Goals[#{game.hscore}]")

          visitorEarnings -= (game.hscore.to_i * game.championship.preferences["match_goal_lost"].delete(",").to_i)
          visitorDescription.push("GoalsAgainst[#{game.hscore}]")
        end

        if game.vscore.to_i > 0 && game.wo == false
          visitorEarnings += (game.vscore.to_i * game.championship.preferences["match_goal_earning"].delete(",").to_i)
          visitorDescription.push("Goals[#{game.vscore}]")

          homeEarnings -= (game.vscore.to_i * game.championship.preferences["match_goal_lost"].delete(",").to_i)
        ...
        homeCards.each do |hCard|
          if hCard.ycard == true
            homeEarnings -= game.championship.preferences["match_yellow_card_loss"].delete(",").to_i
            homeCardsY += 1
          elsif hCard.rcard == true
            homeEarnings -= game.championship.preferences["match_red_card_loss"].delete(",").to_i
            homeCardsR += 1
          end
        end
        if homeCardsY > 0
        ...
        visitorCards.each do |vCard|
          if vCard.ycard == true
            visitorEarnings -= game.championship.preferences["match_yellow_card_loss"].delete(",").to_i
            visitorCardsY += 1
          elsif vCard.rcard == true
            visitorEarnings -= game.championship.preferences["match_red_card_loss"].delete(",").to_i
            visitorCardsR += 1
          end
        end
        if visitorCardsY > 0
