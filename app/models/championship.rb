class Championship < ApplicationRecord
  ...
  def self.types(type = nil)
    ...
    if type
      return championship_types.select {|championship_type| championship_type[:type] == type }
    else
      return championship_types
    end
  end
  ...
  def self.getStatus(championship_id)
    ...
    return status[:"#{championship.status}"]
  end

  def self.getGoalers(championship)
    goalers = PlayerSeason.joins('LEFT OUTER JOIN "club_games" ON "club_games"."player_season_id" = "player_seasons"."id"').where( club_games: { game_id: Game.where(championship_id: championship.id, status: 4) } ).includes(:player).select("player_seasons.id, player_seasons.player_id, COUNT(player_seasons.id) AS goals").group(:id).order("goals desc")
    return goalers
  end

  def self.getAssisters(championship)
    assisters = PlayerSeason.joins('LEFT OUTER JOIN "club_games" ON "club_games"."assist_id" = "player_seasons"."id"').where( club_games: { game_id: Game.where(championship_id: championship.id, status: 4) } ).includes(:player).select("player_seasons.id, player_seasons.player_id, COUNT(player_seasons.id) AS assists").group(:id).order("assists desc")
    return assisters
  end
