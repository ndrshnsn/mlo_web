class AppServices::Games::Earning < ApplicationService
  ...
  def pay_earnings
    ...
    if @game.hscore.to_i > @game.vscore.to_i
      homeEarnings += @game.championship.match_winning_earning
      homeDescription.push("Win")
      visitorEarnings -= @game.championship.match_lost_earning
      visitorDescription.push("Lost")
    elsif @game.hscore.to_i < @game.vscore.to_i
      homeEarnings -= @game.championship.match_lost_earning
      homeDescription.push("Lost")
      visitorEarnings += @game.championship.match_winning_earning
      visitorDescription.push("Win")
    elsif @game.hscore.to_i == @game.vscore.to_i
      homeEarnings += @game.championship.match_draw_earning
      homeDescription.push("Draw")
      visitorEarnings += @game.championship.match_draw_earning
    ...
    if @game.subtype == 3
      ...
      homeGoals.each do |hGoals|
        if hGoals[1] >= 3
          homeEarnings += game.championship.hattrick_earning.cents
      ...
      visitorGoals.each do |vGoals|
        if vGoals[1] >= 3
          visitorEarnings += @game.championship.hattrick_earning
          visitorDescription.push("HatTrick[#{vGoals[0]}]")
        end
      end

      if @game.hscore.to_i > 0 && @game.subtype != 3
        homeEarnings += (@game.hscore.to_i * @game.championship.match_goal_earning)
        homeDescription.push("Goals[#{@game.hscore}]")
        visitorEarnings -= (@game.hscore.to_i * @game.championship.match_goal_lost)
        visitorDescription.push("GoalsAgainst[#{@game.hscore}]")
      end

      if @game.vscore.to_i > 0 && @game.subtype != 3
        visitorEarnings += (@game.vscore.to_i * @game.championship.match_goal_earning)
        visitorDescription.push("Goals[#{@game.vscore}]")
        homeEarnings -= (@game.vscore.to_i * @game.championship.match_goal_lost)
      ...
      homeCards.each do |hCard|
        if hCard.ycard == true
          homeEarnings -= @game.championship.match_yellow_card_loss
          homeCardsY += 1
        elsif hCard.rcard == true
          homeEarnings -= @game.championship.match_red_card_loss
          homeCardsR += 1
        end
      end
      ...
      visitorCards.each do |vCard|
        if vCard.ycard == true
          visitorEarnings -= @game.championship.match_yellow_card_loss
          visitorCardsY += 1
        elsif vCard.rcard == true
          visitorEarnings -= @game.championship.match_red_card_loss
          visitorCardsR += 1
        end
      end
